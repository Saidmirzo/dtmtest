...
class WebAdvertisingBloc
  ...
  WebAdvertisingBloc({required this.advertisingRepository}) : super(const WebAdvertisingState()) {
  ...
  _uploadIMageEvent(UploadImageEvent event, emit) async {
    ...
    final result = await advertisingRepository.uploadImage(event.byte, event.name);
    result.fold(
      (l) => emit(
        state.copyWith(
